...
public class RefactoringRecipeV2 {
	...
	public static void main(String[] args) throws Exception {
		...
		String outputFilePath = "C:\\Savencia\\Output\\output_refactoredRecipecode.xml";
//        String input = "C:\\Savencia\\Input\\Consolidated_Catalog_Products_QUAL_Mar26.txt";
        ...
        String log4jpropertie = "C:\\Savencia\\Config\\log4j.properties";
		 DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
	        factory.setNamespaceAware(true); // recommended
	        DocumentBuilder builder = factory.newDocumentBuilder();
	        Document doc = builder.newDocument();
	        //Creating the STEP-ProductInformation root with attributes
	        Element root = createStepProductInformationRoot(
	                doc,
	                "Global",                    // ContextID
	                "Global",                    // ExportContext
	                "2026-02-16 13:27:25",       // ExportTime
	                "false",                     // UseContextLocale
	                "Main"                       // WorkspaceID
	        );
	        doc.appendChild(root);
	        appendConfigFile(doc, root, Qualifer);
	       // appendReplacementRules(doc, root);
		Document doc1=createXMLDocument(doc, root,Qualifer,input,Config,propertieFile,sitevalue,contextvalue);
		writeXMLToFile(doc1, outputFilePath);		
	 }
	
	 private static Element createStepProductInformationRoot(
	            Document doc,
	            String contextId,
	            String exportContext,
	            String exportTime,
	            String useContextLocale,
	            String workspaceId
	    ) {
	        Element root = doc.createElement("STEP-ProductInformation");
	        root.setAttribute("ContextID", contextId);
	        root.setAttribute("ExportContext", exportContext);
	        root.setAttribute("ExportTime", exportTime);
	        root.setAttribute("UseContextLocale", useContextLocale);
	        root.setAttribute("WorkspaceID", workspaceId);
	        return root;
	    }
	static String[] packattributes =null;
	static String[] France={"fr-FR","fr-BE","fr-CA","fr-CH","fr-CI","fr-NG","fr-GA","fr-CG","fr-RE","fr-LU","fr-ML","fr-NE","fr-CM",
			"fr-DZ","fr-BJ","fr-BF","fr-BI","fr-KM","fr-CD","fr-DJ","fr-DJ","fr-GQ","fr-MG","fr-MG","fr-MC","fr-CF","fr-RW","fr-SN","fr-SC","fr-TD","fr-TG","fr-TN","fr-VU"};
			static String[] German={"de-DE","de-BE","de-CH","de-AT","de-NG","de-LU","de-LI"};
			static String[] Arabic={"ar-AE","ar-SA","ar-EG","ar-MA","ar-KW","ar-TN","ar-DZ","ar-IL","ar-LY","ar-IQ","ar-BH","ar-MR","ar-YE","ar-KM","ar-DJ",
					"ar-ER","ar-PS","ar-QA","ar-SO","ar-SD","ar-TD"};
			static String[] Chinese={"ch-SG","ch-CH","zh-CN","zh-HK"};
			static String[] English={"en-US","en-BE","en-GB","en-IN","en-ID","en-CA","en-IE","en-SG","en-PH","en-HK","en-NG","en-AU","en-NZ","en-GH","en-ZA",
			...
			static String[] Spanish ={"es-AR","es-ES","es-CL","es-UY","es-DO","es-MX","es-BO","es-CO","es-CR","es-EC","es-GT","es-GQ","es-HN","es-NI",
					"es-PA","es-PY","es-SV","es-VE"};
			static String[] Italian ={"it-IT","it-CH","it-SM"};
			static String[] Portuguese={"pt-BR","pt-PT","pt-AO","pt-CV","pt-GQ","pt-GW","pt-MZ","pt-ST","pt-TL"};
			static String[] Dutch={"nl-NL","nl-BE"};
			static String[] Greek={"el-GR","el-CY"};
			static String[] Ro={"ro-RO","ro-MD"};static String[] Ru={"ru-RU","ru-KZ","ru-BY","ru-KG"};
			static String[] Sr={"sr-RS","sr-BA","sr-MNE"}; static String[] Sv={"sv-SE","sv-FI"};
			static String[] Hr={"hr-BA","hr-HR"}; static String[] Ps={"ps-AF","ps-PK"};static String[] Sw={"sw-TZ","sw-UG"};

	private static Document createXMLDocument(Document doc, Element root,String Qualifer, String input,String Config,String propertieFile,String sitevalue,String contextvalue) throws Exception {
		...
		try (BufferedReader reader = new BufferedReader(
			...
			while ((line = reader.readLine()) != null && !line.isEmpty()) {
				...
				if (parts.length == 5) {
					...
					if (pCode.equals("IsPackagingMarkedWithIngredients")|| attribute.equals("ingredientStatement")|| pCode.replaceAll("#\\d+","").equals("nutritionalClaimNonNutrientElement")|| pCode.replaceAll("#\\d+","").equals("nutritionalClaimType")) {
						dataMap.computeIfAbsent(id, k -> new ArrayList<>())
								.add(new String[] { group, attribute, pCode, value });
...
//              }
 		...
 		for (String[] detail : productDetails) {
			...
			String value = detail[2];
			 lValue = detail[3];
			if(value.equals("linkedTradeItem")){
		...
		RecipeBuildContext recipeContext = recipeXmlGen(
				doc,
				productsElement,
				entities,
				recipeMap,
		...
		String recipeID = recipeContext.recipeID;
		String packagingID = null;
		String RpeVariantServingSizeId = recipeContext.RpeVariantServingSizeId;
		String PID = recipeContext.PID;
		HashMap<String,String> compositionIndex = recipeContext.compositionIndex;
		HashMap<String,String> recipekey = recipeContext.recipekey;
		HashMap<String,String> recipeunity = recipeContext.recipeunity;
		HashMap<String,String> sSPrepar = recipeContext.sSPrepar;
		packageXmlGen(doc, productsElement,input,Config);		
...
//					 compositionIndex.clear();
			...
			//Package reference creating.
			if (TradeItemPack.get(productId) != null) {				
...
*/           varientElement3.setAttribute("AttributeID", "Att_Does_Trd_itm_Cont_Pes");
		...
		Element PromometaData = doc.createElement("MetaData");
		...
		Element packageRefKV = doc.createElement("KeyValue");
         ...
         Element sTOSiteclassification = doc.createElement("ClassificationReference");     
         Element hSELSiteclassification = doc.createElement("ClassificationReference");
         Element hPACSiteclassification = doc.createElement("ClassificationReference");      
         Element tRASitemetaData = doc.createElement("MetaData");
         Element cUTSitemetaData = doc.createElement("MetaData");
         Element sTOSitemetaData = doc.createElement("MetaData");
     Element hSELSitemetaData = doc.createElement("MetaData");
         Element hPACSitemetaData = doc.createElement("MetaData");		 
			tRASiteclassification.appendChild(tRASitemetaData);
			cUTSiteclassification.appendChild(cUTSitemetaData);		
			Element hSELcrossValue = doc.createElement("Value");
			Element hSELcrossValue_1= doc.createElement("Value");
			Element hSELcrossValue_2= doc.createElement("Value");
			Element hSELcrossValue_3= doc.createElement("Value");
			Element hSELcrossValue_4= doc.createElement("Value");	
			if(UserTypeID.equals("Savencia_Case")){			
				productElement.appendChild(name);
			Element packageRef = doc.createElement("ProductCrossReference");			
			Element sSname = doc.createElement("KeyValue");	
			...
			String	SCaseID =SavenciaCaseID.get(productId);
			String traditemcode=ProdId.get(SCaseID);			
		...
		if(UserTypeID.equals("Savencia_Pallet")){			
			...
			String traditemcode=ProdId.get(SPalletID);
		...
		//	Element DSconstant_1 = doc.createElement("Value");
			...
			if(UserTypeID.equals("Savencia_Transport_Load")){
				...
				//System.out.println("Savencia_Transport_Load-------------"+productId);
					productElement.appendChild(name);					
					String	transportLoadID =SavenciaCaseID.get(productId);
				//	System.out.println("transportLoadID-------------"+transportLoadID);
					String traditemcode=ProdId.get(transportLoadID);
					Set<String> sds= SDSmap.get(productId);
					Iterator<String> iterator = sds.iterator();
				//	System.out.println("traditemcode-------------"+traditemcode);
					Element sPname = doc.createElement("KeyValue");
					sPname.setAttribute("KeyID", "YODA_Key");
					//sPname.setTextContent(SPalletID+"001");
					sPname.setTextContent(productId);
			...
			if(UserTypeID.equals("Savencia_Pack")){
				...
				String traditemcode=ProdId.get(savenciaPackID);
				Set<String> sds= SDSmap.get(productId);
				Iterator<String> iterator = sds.iterator();
				Element sPname = doc.createElement("KeyValue");
				sPname.setAttribute("KeyID", "YODA_Key");
			...
			//	TradeItemPackRefElement_mM.appendChild(sMMValue);
					}
				}
			...
			//	Element MMconstant_1 = doc.createElement("Value");
		...
		HashSet<String> contractDate = new HashSet<>();
		Element ContractDat = doc.createElement("Value");
		Element MaxAge = doc.createElement("Value");
		...
		String Customclssification = null;
		String CustomclssificationAttribute= null;
		String[] marketname={"BE","NL","DE","AT","CH"};
		String CustomclssificationType = null;
		String Vpack=null;
...
//					 }
		...
		/*------------------------MR50 Endning---------------------------------*/			
					...
					if (fullPath.equals(Configpath) && category.equals("LOVLV")) {
						//Element mValueGroup = doc.createElement("ValueGroup");
//						System.out.println("LOVLV----------"+attrib +"-------------" +lValue);
						if (!valueElements.containsKey(productId)) {
							valueElements.put(productId, new ArrayList<>());
						}
						Element valueElement = doc.createElement("Value");
			        ...
			        /*-----------------UOM-------------------------------------*/
			        		...
			        		if(( fullPath.equals(Configpath) && category.equals("UOM"))){	
			                  ...
			                  for (String productID : descriptions.keySet()) {
			                      ...
			                      for (Map.Entry<String, String> entry3 : descs.entrySet()) {
		   ...
		   /*--------------------------------MR33 End------------------------------*/
				...
				/**	dont modify below four line code. its generating proudct variant**/			
					...
					if ((fullPath.equals(Configpath) && category.equals("LANGONLY"))) {
						...
						for (Map.Entry<String, String> lang : lan.entrySet()) {
						//	System.out.println("Inside ---------"+lang.getKey());
						if(!"fr".equals(lang.getKey())){
						Element Lvalue = doc.createElement("Value");								
						Lvalue.setAttribute("QualifierID", "en-US");
...
private static String userTypeID(String attribute) {
	...
	private static class RecipeBuildContext {
		String recipeID;
		String RpeVariantServingSizeId;
		String PID;
	...
	private static RecipeBuildContext recipeXmlGen(
			Document doc,
			Element productsElement,
			Element entities,
			Map<String, List<String[]>> recipeMap,
			Map<String, List<Element>> valueElements,
			Map<String, Map<String, String>> descriptions,
			Map<String, String> ProdId,
			Map<String, String> grade,
			Map<String, String> SCULMapping,
			Map<String, String> RecipeID,
			Map<String, String> groupTechnology,
			Map<String, String> marquePrincipale,
			Map<String, String> receipeValidatiion,
			Map<String, String> servingsize,
			Map<String, String> declaredUnity,
			Map<String, String> fatPercode,
			List<String[]> att
	) throws Exception {
		String entity = null;
		String category = null;
	   ...
	   /*--------------------------------end----------------------------------*/				
					...
					if ((fullPath.equals(Configpath) && category.equals("RECPL"))) {
						...
						String descJson = lValue.substring(1, lValue.length() - 2); // Remove brackets					
						String regex = "\\{language\\s*:\\s*\"(.*?)\",\\s*text\\s*:\\s*\"(.*?)\"}";
				        Pattern pattern = Pattern.compile(regex);
				        Matcher matcher = pattern.matcher(lValue);
				        Element valueGroup = doc.createElement("ValueGroup");
 ...
 //Unique receipe 
		...
		RecipeBuildContext recipeContext = new RecipeBuildContext();
		recipeContext.recipeID = recipeID;
		recipeContext.RpeVariantServingSizeId = RpeVariantServingSizeId;
		recipeContext.PID = PID;
		recipeContext.compositionIndex = compositionIndex;
		recipeContext.recipekey = recipekey;
		recipeContext.recipeunity = recipeunity;
		recipeContext.sSPrepar = sSPrepar;
		return recipeContext;
	}

	private static Element packageXmlGen(Document doc, Element productsElement,String input,String Config) throws Exception {
		HashSet<String> uniquPackageID= new HashSet<>();
		Map<String, String> lienEmballage = new HashMap<>();
		Map<String, String> grade = new HashMap<>();
		Map<String, Map<String, String>> dataM = new HashMap<>();
		Map<String, List<String[]>> packagingMap = new HashMap<>();
		Map<String, String> PackageNo = new HashMap<>();
		...
		try (BufferedReader reader = new BufferedReader(
			...
			while ((line = reader.readLine()) != null && !line.isEmpty()) {
				...
				if (parts.length == 5) {
					...
					String pCode = parts[3];
					String value = parts[4];
					if(value.equals("BEO") || value.equals("EVI")){
						value="FR";
					}
					if(pCode.equals("targetMarketCountryCode") && value.equals("PC")  ){
						value="FR";	
				 ...
				 for (Map.Entry<String, Map<String,String>> entry : dataM.entrySet()) {
		             ...
		             String productId = entry.getKey();
		             Map<String,String> productDetails = entry.getValue();
		             String grad = productDetails.getOrDefault("grade", "Unknown");
		              grade.put(productId, grad);
		              }
				 for (Map.Entry<String, Map<String,String>> entry : dataM.entrySet()) {
			         	
		             String productId = entry.getKey();
		             Map<String,String> productDetails = entry.getValue();
		            String typeProduits = productDetails.getOrDefault("Types Produits", "Unknown");            
		             SCULMapping.put(productId, typeProduits); 
		             String packaingValidatiion = productDetails.getOrDefault("itemdisplayNameP", "NO");
					 packageValidation.put(productId, packaingValidatiion); 
		             String receipeValidate = productDetails.getOrDefault("itemdisplayNameR", "NO"); 
				 ...
				 for (Map.Entry<String, Map<String,String>> entry : dataM.entrySet()) {
		             ...
		             String productId = entry.getKey();
		             Map<String,String> productDetails = entry.getValue();
		             String ptc = productDetails.getOrDefault("packagingTypeCode", "Unknown");
		               if(!ptc.equals("Unknown")){
		            	  packagingTypeCode.put(productId, ptc);
		              }else{
				 ...
				 for (Map.Entry<String, Map<String,String>> entry : dataM.entrySet()) {
		             ...
		             String productId = entry.getKey();
		             Map<String,String> productDetails = entry.getValue();
		             String lienEmbal = productDetails.getOrDefault("lienEmballage", "Unknown");            
		              lienEmballage.put(productId, lienEmbal);
		              }
				 for (Map.Entry<String, Map<String, String>> entry : dataM.entrySet()) {

						String productId = entry.getKey();
						Map<String, String> productDetails = entry.getValue();		
						String itemDescr = productDetails.getOrDefault("tradeItemUnitDescriptor", "Unknown");					
						itemDesc.put(productId, itemDescr);			
					}
		...
		for (Map.Entry<String, List<String[]>> entry : packagingMap.entrySet()) {
			String ErrorPath = null;
			String packagingID = null;
			...
			String packid=productId.substring(0, productId.length()-2);
				uniquPackageID.add(lienEmballage.get(packid));
			if("GRA02".equals(grade.get(productId.substring(0, productId.length()-2)))){
				packagingID="SP"+productId.substring(0, productId.length()-4);
				TradName =itemDesc.get(productId.substring(0, productId.length()-2));
			...
			packagingID= lienEmballage.get(productId.substring(0, productId.length()-2));			
			TradName =itemDesc.get(productId.substring(0, productId.length()-2));			
			}
			
			if(TradName.equals("EA")){
				TradName = "BA";
			}
			if(TradName.equals("PK")){
				TradName = "PA";
			}		
			...
			List<String[]> productDetails = entry.getValue();			
			String UserTypeID = userTypeID(PackageNo.get(productId));		    
			...
			productElement.setAttribute("ParentID", "PPH_"+TradName+"_"+packagingTCode);			
			}else{
				productElement.setAttribute("ParentID", "PPH_"+TradName+"_PUG");	
			}
			...
			packagingID= lienEmballage.get(SubPackagingID);
...
/*-----------------PUOM-------------------------------------*/
	...
	private static void appendConfigFile(Document targetDoc, Element parent, String xmlFile) throws Exception {
        DocumentBuilderFactory f = DocumentBuilderFactory.newInstance();
        f.setNamespaceAware(true);
        DocumentBuilder b = f.newDocumentBuilder();

        Document ext = b.parse(xmlFile);

        // Import the external root into our target document
        Node importedRoot = targetDoc.importNode(ext.getDocumentElement(), true); // deep copy

        // If you need it as the *first* child right after header, use insertBefore
        if (parent.getFirstChild() != null) {
            parent.insertBefore(importedRoot, parent.getFirstChild());
        } else {
            parent.appendChild(importedRoot);
        }
        appendReplacementRules(targetDoc, parent);
    }
		 private static void appendReplacementRules(Document doc, Element root) {
	        // Create container elements
	        Element replacementRules = doc.createElement("ReplacementRules");
	        Element products = doc.createElement("Products");

	        // Add ReplaceDataContainers entries
	        products.appendChild(newReplaceDataContainers(doc, "Dct_Microbio_Criteria"));
	        products.appendChild(newReplaceDataContainers(doc, "Dct_Physico_Chemical_Criteria"));
	        products.appendChild(newReplaceDataContainers(doc, "Dct_Additional_Microbiological_Criteria"));
	        products.appendChild(newReplaceDataContainers(doc, "Dct_PackagingMaterial"));
	        products.appendChild(newReplaceDataContainers(doc, "Dct_Composite_Packaging_Material"));
	        products.appendChild(newReplaceDataContainers(doc, "Dct_Media"));
	        products.appendChild(ReplaceCrossReferences(doc, "Rpp_TradeItem_Pack"));
	        products.appendChild(ReplaceCrossReferences(doc, "Rpp_Variant_Recipe"));
	        products.appendChild(ReplaceClassificationReferences(doc, "Rpc_Market_Link"));
	        products.appendChild(ReplaceClassificationReferences(doc, "Rpc_Site_Role"));
		
//	        // Add empty elements
//	        products.appendChild(doc.createElement("ReplaceCrossReferences"));
//	        products.appendChild(doc.createElement("ReplaceClassificationReferences"));

	        // Assemble subtree
	        replacementRules.appendChild(products);

	        // Insert as the first child under the root (right after header)
	        Node first = root.getFirstChild();
	        if (first != null) {
	            //root.insertBefore(replacementRules, first);
	        	root.appendChild(replacementRules);
	        } else {
	            root.appendChild(replacementRules);
	        }
	    }

	    private static Element newReplaceDataContainers(Document doc, String dataContainerTypeId) {
	        Element e = doc.createElement("ReplaceDataContainers");
	        e.setAttribute("DataContainerTypeID", dataContainerTypeId);     
	        return e;
	    }
	    
	    private static Element ReplaceCrossReferences(Document doc, String dataContainerTypeId) {
	        Element e = doc.createElement("ReplaceCrossReferences");
	        e.setAttribute("ReferenceTypeID", dataContainerTypeId);     
	        return e;
	    }
	    
	    private static Element ReplaceClassificationReferences(Document doc, String dataContainerTypeId) {
	        Element e = doc.createElement("ReplaceClassificationReferences");
	        e.setAttribute("ReferenceTypeID", dataContainerTypeId);     
	        return e;
	    }
